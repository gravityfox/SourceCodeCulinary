...
public class WorldGenSalt implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
        ...
        if(b.biomeName==BiomeGenBase.desert.biomeName) {
        	for(int l = 0; l < 6; l++)
            {
                    int i1 = chunkX*16 + random.nextInt(16);
                    int j1 = random.nextInt(20)+40;
                    int k1 = chunkZ*16 + random.nextInt(16);
                    (new WorldGenMinable(CuisineMod.saltblock.blockID, 3)).generate(world, random, i1, j1, k1);
            }
